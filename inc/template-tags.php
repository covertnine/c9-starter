	...
	function c9_entry_footer()
	{
		...
		if ('post' === get_post_type()) {
			...
			if ($categories_list && c9_categorized_blog()) {
				printf('<span class="cat-links">' . esc_html__('Posted in %1$s', 'c9-work') . '</span>', $categories_list);
			}
			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list('', esc_html__(', ', 'c9-work'));
			if ($tags_list) {
				printf('<span class="tags-links">' . esc_html__('Tagged %1$s', 'c9-work') . '</span>', $tags_list);
			}
		}

	...
	function c9_entry_footer()
	{
		...
		if ('post' === get_post_type()) {
			...
			if ($categories_list && c9_categorized_blog()) {
				printf('<span class="cat-links">' . esc_html__('Posted in %1$s', 'c9-starter' ) . '</span>', $categories_list); // WPCS: XSS OK.
			}
			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list('', esc_html__(', ', 'c9-starter' ));
			if ($tags_list) {
				printf('<span class="tags-links">' . esc_html__('Tagged %1$s', 'c9-starter' ) . '</span>', $tags_list); // WPCS: XSS OK.
			}
		}

...
if (!function_exists('c9_posted_on')) {
	function c9_posted_on()
	{
		...
		$posted_on = sprintf(
			esc_html_x('Posted on %s', 'post date', 'c9-music'),
		...
		if (get_theme_mod('c9_author_visible') === "show") {
			$byline = sprintf(
				esc_html_x('by %s', 'post author', 'c9-music'),
...
if (!function_exists('c9_entry_footer')) {
	function c9_entry_footer()
	{
		...
		if (!is_single() && !post_password_required() && (comments_open() || get_comments_number())) {
			...
			comments_popup_link(esc_html__('Leave a comment', 'c9-music'), esc_html__('1 Comment', 'c9-music'), esc_html__('% Comments', 'c9-music'));
			echo '</span>';
		}
		edit_post_link(
			sprintf(
				/* translators: %s: Name of current post */
				esc_html__('Edit %s', 'c9-music'),

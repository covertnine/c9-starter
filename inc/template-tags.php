...
if (!function_exists('c9_posted_on')) {
	function c9_posted_on()
	{
		...
		$posted_on = sprintf(
			esc_html_x('Posted on %s', 'post date', 'c9-togo' ),
		...
		if (get_theme_mod('c9_author_visible') === "show") {
			$byline = sprintf(
				esc_html_x('by %s', 'post author', 'c9-togo' ),
...
if (!function_exists('c9_entry_footer')) {
	function c9_entry_footer()
	{
		...
		if ('post' === get_post_type()) {
			...
			$categories_list = get_the_category_list(esc_html__(', ', 'c9-togo' ));
			if ($categories_list && c9_categorized_blog()) {
				printf('<span class="cat-links">' . esc_html__('Posted in %1$s', 'c9-togo' ) . '</span>', $categories_list); // WPCS: XSS OK.
			}
			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list('', esc_html__(', ', 'c9-togo' ));
			if ($tags_list) {
				printf('<span class="tags-links">' . esc_html__('Tagged %1$s', 'c9-togo' ) . '</span>', $tags_list); // WPCS: XSS OK.
			}
		}
		if (!is_single() && !post_password_required() && (comments_open() || get_comments_number())) {
			echo '<span class="comments-link">';
			comments_popup_link(esc_html__('Leave a comment', 'c9-togo' ), esc_html__('1 Comment', 'c9-togo' ), esc_html__('% Comments', 'c9-togo' ));
			echo '</span>';
		}
		edit_post_link(
			sprintf(
				/* translators: %s: Name of current post */
				esc_html__('Edit %s', 'c9-togo' ),

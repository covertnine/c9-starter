...
if (file_exists(locate_template('client/inc/footer.php'))) {
	...
	$c9_footer_default = '&copy; ' . date("Y") . ' ' . get_bloginfo('name') . '. <a href="https://www.covertnine.com" title="' . esc_attr__('Web design company in Chicago', 'c9-music') . '" target="_blank">' . esc_html__('WordPress Website design by COVERT NINE', 'c9-music') . '</a>.';
...
<div id="fullscreensearch">
	<form role="search" method="get" id="fullscreen" action="/">
		<div>
			<span class="sr-only"><?php esc_html_e('Search for:', 'c9-music'); ?></span>
			<input type="search" class="search-field" name="s" value="" tabindex="0" placeholder="<?php esc_attr_e('Search...', 'c9-music'); ?>" />
			<button type="submit" class="btn"><?php esc_html_e('Search', 'c9-music'); ?></button>
		</div>
	</form>
	<button type="button" class="search-close"><i class="fa fa-close"></i><span class="sr-only"><?php esc_html_e('Close', 'c9-music'); ?></span></button>
</div>
